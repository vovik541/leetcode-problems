package dev.vkh.solutions.medium.wordSquaresII;
    ...
    for (String topWord : words) {
      for (String bottomWord : words) {
        ...
        List<String> leftCandidates =
            wordsByFirstAndLastChar.getOrDefault(
                makeKey(requiredLeftFirstChar, requiredLeftLastChar), List.of());
        List<String> rightCandidates =
            wordsByFirstAndLastChar.getOrDefault(
                makeKey(requiredRightFirstChar, requiredRightLastChar), List.of());
        ...
        for (String leftWord : leftCandidates) {
          ...
          for (String rightWord : rightCandidates) {
            if (rightWord.equals(topWord)
                || rightWord.equals(bottomWord)
                || rightWord.equals(leftWord)) {
              continue;
            }
