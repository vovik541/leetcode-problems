package dev.vkh.solutions.medium.magicSquaresInGrid;
...
class Solution {
  ...
  private int calculateRowSum(int[][] grid, int row, int leftColumn) {
    return grid[row][leftColumn] + grid[row][leftColumn + 1] + grid[row][leftColumn + 2];
  }

  private int calculateColumnSum(int[][] grid, int topRow, int column) {
    return grid[topRow][column] + grid[topRow + 1][column] + grid[topRow + 2][column];
  }
